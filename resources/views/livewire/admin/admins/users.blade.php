<section class="w-full">
    <div class="flex items-center justify-between pr-5">
        ...
        <div class="flex gap-2">
            ...
            <button x-on:click='$dispatch("open-modal","add-admin")' class="gap-1 bg-white shadow hover:bg-blue-500 hover:text-white btn">
        ...
        <table class="table w-full table-compact">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $user->email }}</td>
                    <td>
                        <button wire:confirm='Are you sure to delete {{ $user->role }} {{ $user->name }}?' wire:click='delete({{ $user }})' class="text-xs text-rose-600">Delete</button>
                    </td>
                </tr>
                @empty
                <tr>
        ...
        <div class="p-6">
            ...
            <form wire:submit='save' class="space-y-2">
                <div class="flex flex-col">
                    ...
                    @error('name') <small class="italic text-rose-600">{{ $message }}</small> @enderror
                </div>

                <div class="flex flex-col">
                    <label>Email</label>
                    <input wire:model='email' type="email" class="border rounded shadow border-black/10" required>
                    @error('email') <small class="italic text-rose-600">{{ $message }}</small> @enderror
                ...
                <div class="flex flex-col">
                    ...
                    @error('role') <small class="italic text-rose-600">{{ $message }}</small> @enderror
                </div>

                <div class="flex flex-col">
                    <label>Password</label>
                    <input wire:model='password' type="password" class="border rounded shadow border-black/10" required>
                    @error('password') <small class="italic text-rose-600">{{ $message }}</small> @enderror
                </div>

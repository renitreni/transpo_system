<section class="w-full pb-7">
    <h1 class="text-2xl font-bold ">Add New Client / إضافة عميل جديد</h1>

    @include('success.success')
    <form wire:submit.prevent='SaveReceipt' class="grid grid-cols-4 mt-14 gap-7">
        <div class="col-span-3 rounded-md shadow-sm">
            <div
                class="p-4  rounded relative before:absolute before:left-0 before:w-full before:h-1 before:content=[''] before:top-0 before:rounded-full before:bg-blue-500">
            ...
            <div class="grid grid-cols-4 gap-3 p-4">
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='PlateNo' autocomplete="off" type="text"
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='PhoneNumber' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                    @error('PhoneNumber')
                        <span class="text-xs text-rose-600">{{ $message }}</span>
                    @enderror
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='CompanyName' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                    @error('CompanyName')
                        <span class="text-xs text-rose-600">{{ $message }}</span>
                    @enderror
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='OfficeAddress' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                    @error('OfficeAddress')
                        <span class="text-xs text-rose-600">{{ $message }}</span>
                    @enderror
                ...
                <div class="flex flex-col justify-center col-span-4">
                    ...
                    <input wire:model='OtherLocation' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                    @error('OtherLocation')
                        <span class="text-xs text-rose-600">{{ $message }}</span>
                    @enderror
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='driver_name' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                    @error('driver_name')
                        <span class="text-xs text-rose-600">{{ $message }}</span>
                    @enderror
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='car_insurance_company' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                    @error('car_insurance_company')
                        <span class="text-xs text-rose-600">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='driver_license_number' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='driver_license_expiry_date' autocomplete="off" type="date"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <input wire:model='resident_iqama_number' autocomplete="off" type="text"
                        class="w-full p-1 bg-transparent border rounded-md border-black/30 focus:outline-blue-400">
                    @error('resident_iqama_number')
                        <span class="text-xs text-rose-600">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div class="rounded-md shadow-sm">
            <div
                class="p-4  rounded relative before:absolute before:left-0 before:w-full before:h-1 before:content-[''] before:top-0 before:bg-blue-500 before:rounded-full">
            ...
            <div class="p-4">
                <div>
                    ...
                    <input wire:input.live.debounce.1000ms='updateWarrantyExpirations' wire:model='OrderDate'
        ...
        <div class="flex w-full overflow-x-auto col-span-full">
            <table
                class="table relative before:absolute before:h-1 before:w-full before:conten=[''] before:top-0 before:bg-blue-600 before:rounded-full">
                <thead>
                    <tr>
                        <th style="background-color: #fcfcfc;" class="w-8">
                            <button wire:click='add_product' type="button"
                                class="w-8 p-1 text-blue-600 duration-300 appearance-none ">
                ...
                <tbody>
                    @forelse ($products as $index => $product )
                        <tr wire:key='{{ $product['Product'] }}'>
                            <th>
                                <button wire:click='remove_product({{ $index }})' type="button"
                                    class="w-8 p-1 duration-300 text-rose-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            ...
                            <td><input required wire:model='products.{{ $index }}.Quantity' type="number"
                                    min="0"
                                    class="w-full p-1 bg-transparent rounded-md border-black/30 focus:outline-blue-400">
                            </td>
                            <td x-data="{ expanded: false }">
                                <input required wire:model='products.{{ $index }}.Product' type="text"
                                    x-on:click="expanded = true" x-on:blur="expanded = false"
                                    x-bind:class="expanded ? 'w-[250px]' : 'w-24'"
                                    class="p-1 bg-transparent rounded-md border-black/30 focus:outline-blue-400">
                            </td>
                            <td><input required wire:model='products.{{ $index }}.Color' type="text"
                                    class="w-full p-1 bg-transparent rounded-md border-black/30 focus:outline-blue-400">
                            </td>
                            <td x-data="{ expanded: false }">
                                <input required wire:model='products.{{ $index }}.ChassisNumber'
                                    type="text" x-on:click="expanded = true" x-on:blur="expanded = false"
                                    x-bind:class="expanded ? 'w-[250px]' : 'w-24'"
                                    class="p-1 bg-transparent rounded-md border-black/30 focus:outline-blue-400">
                            </td>
                            <td><input required wire:model='products.{{ $index }}.YearModel' type="number"
                                    placeholder="years" min="0"
                                    class="w-full p-1 bg-transparent rounded-md border-black/30 focus:outline-blue-400">
                            </td>
                            <td>
                                <input required
                                    wire:input.live.debounce.1000ms='getDateViaMonths({{ $index }})'
                                    wire:model='products.{{ $index }}.WarrantyPeriod' type="number"
                                    placeholder="months" min="0"
                                    class="w-full p-1 bg-transparent rounded-md border-black/30 focus:outline-blue-400">
                            </td>
                            <td><input required wire:model='products.{{ $index }}.WarrantyExpiration'
                    ...
                    @empty
                        <tr>
                            ...
                            @else
                                @if (session('no_date'))
                                    <td colspan="12"><span
                                            class="text-sm text-rose-500">{{ session('no_date') }}</span></td>
                                @else
        ...
        <div class="flex justify-end gap-3 col-span-full">
            <button type="button"
                onclick="window.location.href='{{ route('admin_ManageDeliveries', ['lang' => 'en']) }}'"
                class="px-4 py-3 text-white duration-200 rounded-md bg-slate-500 hover:bg-slate-700">Cancel</button>
            <button
                class="px-4 py-3 font-bold duration-300 bg-blue-600 rounded-md min-w-min hover:bg-blue-800 text-slate-200"
                type="submit">
                <span wire:target='SaveReceipt' wire:loading.remove>Add Client</span>
                <span wire:target='SaveReceipt' wire:loading>Loading...</span>
            </button>
        </div>

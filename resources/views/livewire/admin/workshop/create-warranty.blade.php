    ...
    <form enctype="multipart/form-data" wire:submit.prevent='create_report' class="{{ $formStyle }}">
                ...
                <div class="flex items-center gap-2">
                    <div class="{{ $inputContainerStyle }}">
                        ...
                        <select wire:model='form.Brand' class="{{ $selectStyle }} {{ $inputStyle }}">
                            ...
                            <option value="SULTANALFOUZANCO">SULTANALFOUZANCO</option>
                        </select>
                    </div>
        ...
        <div class="{{ $boxStyle }} col-span-full">
            ...
            <div class="grid grid-cols-5 mt-5 gap-x-2">
                ...
                <div class="{{ $inputContainerStyle }}">
                    <label>Status</label>
                    <div class="flex items-center justify-between p-2.5 border border-black/10 bg-white rounded-md shadow">
                        <span>{{ $form->Status ? "Working":"Need Repair" }}</span>
                        <input wire:model.live='form.Status' type="checkbox" class="border-black/30 switch switch-ghost-primary" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="{{ $boxStyle }} col-span-full">
            ...
            <div  class="flex items-center gap-4 mt-6 text-sm">
                <div class="flex flex-col">
                    ...
                    <input wire:model='form.Images' accept="image/*,video/*" type="file" multiple class="input-file" />
                </div>
                <div class="flex flex-col">
            ...
            <textarea wire:model='form.Report' required placeholder="Type something..." class="w-full mt-2 rounded-lg shadow border-black/10"  rows="5"></textarea>
        </div>


        <div class="flex justify-end col-span-full gap-x-2">
            <button type="button" onclick="window.location.href='{{ route('admin_ManageWarranty',['lang'=>'en']) }}'" class="btn hover:bg-slate-300">Cancel</button>
            <button wire:target='create_report'  wire:loading.attr="disabled" type="submit"  class="gap-2 text-white bg-blue-500 disabled:cursor-not-allowed disabled:pointer-events-none disabled:bg-slate-200 btn">
